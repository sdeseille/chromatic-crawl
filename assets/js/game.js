...
function defeatGuardian(room) {
  ...
  if (room.type === 'b') {
    ...
    startVictoryCutscene(room.color); // pot -> burst -> raise -> crystalgrow -> hold, then advanceOrWin()
  } else {
...
loadImage('leprechaun_64x16.png').then(img => leprechaunImg = img);
loadImage('gold_pot_32x16.png').then(img => potImg = img);
...
// on leprechaun_64x16.png / gold_pot_32x16.png:
//   'p' (pot)         — pot of gold sits out (frame 0), leprechaun blinks
//                        between his two idle frames (0/1)
//   'b' (burst)       — pot flashes to its sparkle-burst frame (1), then gone
//   'r' (raise)       — leprechaun swings arms-down to arms-raised (2)
//   'g' (crystalgrow) — leprechaun holds his final arms-up frame (3) while a
//                        tiny crystal in the reclaimed color scales in above
//                        his head
//   'h' (hold)        — crystal held fully visible for a beat
// then finishCutscene() hands off to advanceOrWin() — the reward itself
// still lands the instant the boss dies (collectColor already ran), but the
// level transition waits for the flourish.
//
...
const CRYSTAL_SCALE = 24 / 14; // diamond path spans 14 units tall -> scales to 24px high, width stays proportional
...
// facets for a cheap glint, outlined. Uses the room/blessing color string
// directly (already a valid CSS color, e.g. 'violet') rather than any hex
// math, since shadeColor() only handles '#rrggbb' input.
function drawColorCrystal(ctx, x, y, color, scale = 3) {
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(scale * CRYSTAL_SCALE, scale * CRYSTAL_SCALE);
  ...
  ctx.globalAlpha = 0.5; // brighter facet = cheap glint
  ctx.fillStyle = '#fff';
  ctx.beginPath();
